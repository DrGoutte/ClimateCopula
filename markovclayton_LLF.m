function [LLF, LL, theta_s0, theta_s1, pf1, pf2, pplus1, pplus2] = markovclayton_LLF(kappa,data,thetabar)
% function [LLF, LL, theta_s0, theta_s1, pf1, pf2, pplus1, pplus2] = markovclayton_LLF(kappa,data,thetabar)
% The negative copula log-likelihood of the Clayton copula 
...
for jj = 2:n;
   ...
   %regime0
   if jj<=10
      psi_s0 =w0 + b.*theta_s0(jj-1) + a.*(mean(abs(u(1:jj-1)-v(1:jj-1))));
   else
      psi_s0 =w0 + b.*theta_s0(jj-1) + a.*(mean(abs(u(jj-10:jj-1)-v(jj-10:jj-1))));
   end
   theta_s0(jj)=0.0001+psi_s0^2;
   
   %regime1
   if jj<=10
      psi_s1 =w1 + b.*theta_s1(jj-1) + a.*(mean(abs(u(1:jj-1)-v(1:jj-1))));
   else
      psi_s1 =w1 + b.*theta_s1(jj-1) + a.*(mean(abs(u(jj-10:jj-1)-v(jj-10:jj-1))));
   end
   theta_s1(jj)=0.0001+psi_s1^2;
...
%Filtered probabilities
    ...
    elseif jj<n %Updating Pr[st=j|It], j=0,1.
        ...
        c1=clayton_pdf(u(jj-1),v(jj-1),theta_s1(jj-1));
        c1=c1.*p1(jj-1);
       
        c0=clayton_pdf(u(jj-1),v(jj-1),theta_s0(jj-1));    
        c0=c0.*p2(jj-1);
...
pplus2=p2t;
    ...
    LL = -1*log((clayton_pdf(u,v,theta_s1)).*pf1+(clayton_pdf(u,v,theta_s0)).*pf2);
    LLF = sum(LL);
