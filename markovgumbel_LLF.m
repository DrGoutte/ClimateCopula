function [LLF, LL, theta_s0, theta_s1, pf1, pf2, pplus1, pplus2] = markovgumbel_LLF(kappa,data,thetabar)
% function [LLF, LL, theta_s0, theta_s1, pf1, pf2, pplus1, pplus2] = markovgumbel_LLF(kappa,data,thetabar)
% The negative copula log-likelihood of the Gumbel copula 
...
%Filtered probabilities
    ...
    elseif jj<n %Updating Pr[st=j|It], j=0,1.
        ...
        c1=gumbel_pdf(u(jj-1),v(jj-1),theta_s1(jj-1));
        c1=c1.*p1(jj-1);
       
        c0=gumbel_pdf(u(jj-1),v(jj-1),theta_s0(jj-1));    
        c0=c0.*p2(jj-1);
...
pplus2=p2t;
    ...
    LL = -1*log((gumbel_pdf(u,v,theta_s1)).*pf1+(gumbel_pdf(u,v,theta_s0)).*pf2);    
    LLF = sum(LL);
