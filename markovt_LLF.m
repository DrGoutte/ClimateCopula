function [LLF, LL, theta_s0, theta_s1, pf1, pf2, pplus1, pplus2] = markovt_LLF(kappa,data,thetabar)
% function [LLF, LL, theta_s0, theta_s1, pf1, pf2, pplus1, pplus2] = markovt_LLF(kappa,data,thetabar)
% The negative copula log-likelihood of the Student-t copula 
...
%		  thetabar = [Rho,DoF], results from estimation of unconditional model
%
...
x = tinv(data(:,1),thetabar(2));
y = tinv(data(:,2),thetabar(2));
...
nu0 = (exp(kappa(7))/(1+exp(kappa(7))))*98 + 2;
nu1 = (exp(kappa(8))/(1+exp(kappa(8))))*98 + 2;

theta_s0=ones(n,1);
theta_s1=ones(n,1);

theta_s0(1)=thetabar(1);
theta_s1(1)=thetabar(1);
...
%Filtered probabilities
    ...
    elseif jj<n %Updating Pr[st=j|It], j=0,1.
        ...
        c1 = tcopula_pdf(u(jj-1),v(jj-1),theta_s1(jj-1),nu1);
        
        c1=c1.*p1(jj-1);
       
        c0 = tcopula_pdf(u(jj-1),v(jj-1),theta_s0(jj-1),nu0);
...
pf2=p2;
    ...
    LL = -1*log((tcopula_pdf(u,v,theta_s1,nu1).*pf1+(tcopula_pdf(u,v,theta_s0,nu0).*pf2)));
    LLF = sum(LL);
